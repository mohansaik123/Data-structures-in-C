...
{ 
	...
	int i = s->n-1; //index is no of keys (n-1 because array starts from 0)
	 ...
	 //if node is leaf
	if (s->leaf == true) 
	{ 	//loop to shit the keys and decrease the index value if keys present are greater than given key
...
struct btree insert(int k,struct btree b) 
...
{ 
	...
	//size is for keeping track of no of nodes present in the que at a particular level 
	...
	{ 	size=count; //the size of ques is no of nodes enqued
		...
		while(size>0){
			...
			count--;//reduce the count of nodes enqued
...
struct btree removekey(int k, struct btree b);
...
{ 
	...
	{ 
		...
		//elsen ecurse on the (idx)th child (has t keys after filling)
		else
...
{ 
	...
	struct btreeNode* cur =s->C[idx+1]; 
	while (!cur->leaf) 
		cur = cur->C[0]; 
...
{ 
	...
	{ 	// if node is the last child merge it with with its previous sibling 
		if (idx != s->n) 
			s=merge(idx,s); 
		// else merge it with next sibiling
		else
			s=merge(idx-1,s); 
	} 
	return s; 
